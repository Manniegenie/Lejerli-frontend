...
const THEME: Record<ToastType, { color: string; bg: string }> = {
  ...
  info:    { color: '#F26522', bg: 'rgba(242,101,34,0.1)' },
};
  ...
  return (
    <Animated.View style={[s.container, { transform: [{ translateY: slideAnim }], opacity: opacityAnim }]} pointerEvents="box-none">
      <TouchableWithoutFeedback onPress={dismiss}>
        <View style={[s.card, { backgroundColor: bg, borderLeftColor: color, shadowColor: colors.text }]}>
          <Text style={[s.title, { color }]} numberOfLines={2}>{title}</Text>
          {message ? <Text style={[s.message, { color: colors.textSecondary }]} numberOfLines={4}>{message}</Text> : null}
        </View>
