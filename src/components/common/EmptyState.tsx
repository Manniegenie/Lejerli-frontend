...
function EmptyIcon({ size }: { size: Size }) {
  const sm = size === 'sm';
  const dim = sm ? 32 : 52;
  const stroke = '#2a2a2a';
  const sw = 1.2;
...
export default function EmptyState({
  ...
  return (
    <View style={[s.wrap, sm ? s.wrapSm : s.wrapMd]}>
      {Illustration ?? <EmptyIcon size={size} />}
      <Text style={[s.message, sm && s.messageSm]}>{message}</Text>
      {sub ? <Text style={[s.sub, sm && s.subSm]}>{sub}</Text> : null}
      {onPress && actionLabel ? (
        <TouchableOpacity style={[s.btn, sm && s.btnSm]} onPress={onPress} activeOpacity={0.7}>
          <Text style={[s.btnText, sm && s.btnTextSm]}>{actionLabel}</Text>
        </TouchableOpacity>
      ) : null}
    </View>
  );
...
const s = StyleSheet.create({
  ...
  message:    { fontSize: 13, color: '#999999', marginTop: 14, textAlign: 'center', letterSpacing: 0.2 },
  messageSm:  { fontSize: 12, marginTop: 10 },

  sub:        { fontSize: 12, color: '#888888', marginTop: 6, textAlign: 'center', lineHeight: 18 },
  subSm:      { fontSize: 11, marginTop: 4 },

  btn:        { marginTop: 20, borderWidth: 1, borderColor: '#2a2a2a', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 6 },
  btnSm:      { marginTop: 12, paddingHorizontal: 14, paddingVertical: 7 },
  btnText:    { fontSize: 13, color: '#bbbbbb', letterSpacing: 0.3 },
