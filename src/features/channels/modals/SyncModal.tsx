...
export default function SyncModal({ visible, exchangeName, onClose, onConfirm }: Props) {
  ...
  const [selectedPreset, setPreset]   = useState<ImportRange>(PRESETS[3]); // default 12M
  ...
  const handleConfirm = async () => {
    ...
    try {
      await onConfirm(
        ...
        { from: selectedPreset.from, to: selectedPreset.to }
      );
      onClose();
    } catch (_) {
    } finally {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={s.sheet}>
        ...
        <Text style={s.subtitle}>Choose what data to import and the historical range</Text>
        ...
        {/* ── Data permissions ── */}
        <Text style={s.sectionLabel}>DATA TO IMPORT</Text>
        ...
        >
          {loading
            ...
            : <Text style={s.confirmText}>Start Import</Text>}
        </TouchableOpacity>
  ...
  return (
    <View style={s.optionRow}>
      ...
      <Switch
        ...
        thumbColor={value ? '#F26522' : '#999999'}
      />
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  subtitle:           { fontSize: 13, color: '#999999', marginBottom: 24 },

  sectionLabel:       { fontSize: 10, color: '#555555', letterSpacing: 1.4, marginBottom: 12 },

  presets:            { flexDirection: 'row', gap: 10, marginBottom: 28 },
  presetChip:         { flex: 1, height: 38, borderRadius: 10, borderWidth: 1, borderColor: '#2a2a2a', alignItems: 'center', justifyContent: 'center', backgroundColor: '#0f0f0f' },
  presetChipActive:   { borderColor: '#F26522', backgroundColor: 'rgba(242,101,34,0.1)' },
  presetText:         { fontSize: 13, fontWeight: '500', color: '#666666' },
  presetTextActive:   { color: '#F26522' },
  ...
  optionSub:          { fontSize: 12, color: '#999999' },
  ...
  cancelText:         { fontSize: 15, color: '#999999' },
});
