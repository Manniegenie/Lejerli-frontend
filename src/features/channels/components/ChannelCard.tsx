...
const EXCHANGE_META: Record<string, { color: string; initials: string }> = {
  ...
  obiex:      { color: '#00C2FF', initials: 'OB' },
    ...
    <View style={s.card}>
      ...
      <View style={s.topRow}>
        {CHANNEL_LOGOS[channel.id]
          ? <Image source={CHANNEL_LOGOS[channel.id]} style={s.iconLogo} resizeMode="contain" />
        ...
        <View style={[s.statusBadge, { backgroundColor: isConnected ? '#14532d' : '#1c1c1c' }]}>
          <View style={[s.statusDot, { backgroundColor: isConnected ? '#22c55e' : '#999999' }]} />
          <Text style={[s.statusText, { color: isConnected ? '#4ade80' : '#bbbbbb' }]}>
...
const s = StyleSheet.create({
  ...
  balanceLabel:      { fontSize: 11, color: '#999999' },
  ...
  disconnectText:    { fontSize: 13, color: '#bbbbbb' },
});
